...
pose_o = pose.Pose()
#show visual to user whatever the media pipe is able to detect
drawing = mp.solutions.drawing_utils

#initializing video capture from webcam
...
while True:
	stime = time.time()
    #returns frames from video captured from webcam
	_, frm = cap.read()

    #returns processed image from pose detection
    #by default OpenCV returns the image in BGR format
    #so it needs to explicitly converted to RGB
    ...
    # here chek if any key down the make it up
	else: 
    ...
    #showing captured frames to user
	cv2.imshow("window", frm) 
    #checking if user presses escape key. 
    #exits if true and destroy all the windows
	if cv2.waitKey(1) == 27:
		cap.release()
